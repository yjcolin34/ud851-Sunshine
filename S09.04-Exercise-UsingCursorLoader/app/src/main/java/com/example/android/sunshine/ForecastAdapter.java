...
//      TODO (8) Display the summary that you created above
    ...
    long dateInMillis = mCursor.getLong(Integer.parseInt(WeatherEntry.COLUMN_DATE));
        /* Get human readable string using our utility method */
    String dateString = SunshineDateUtils.getFriendlyDateString(mContext, dateInMillis, false);
        /* Use the weatherId to obtain the proper description */
    int weatherId = mCursor.getInt(Integer.parseInt(WeatherEntry.COLUMN_WEATHER_ID));
    String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);
        /* Read high temperature from the cursor (in degrees celsius) */
    double highInCelsius = mCursor.getDouble(Integer.parseInt(WeatherEntry.COLUMN_MAX_TEMP));
        /* Read low temperature from the cursor (in degrees celsius) */
    double lowInCelsius = mCursor.getDouble(Integer.parseInt(WeatherEntry.COLUMN_MIN_TEMP));
...
//      String weatherForDay = mWeatherData[adapterPosition];
      String weatherForDay = String.valueOf(weatherSummary.getText());
